class Trait {
  ...
  public Trait(Player p, boolean DETAILED, float startingHunger, float startingThirst, float[] parentStats) {
    ...
    if (parentStats != null) {
      for (int i = 0; i < 5; i++) {
        float mutation = random(-0.05, 0.05); // +/- 5% drift
        stats[i] = constrain(parentStats[i] + mutation, 0.0, 1.0);
      }
    } else {
      // Primordial population baseline
      for (int i = 0; i < 5; i++) {
        stats[i] = random(0.35, 0.65); //randomly generated genetics
      }
    }
...
// Stamina speed boost (+7% speed at 0, -7% top speed at 1.0)
  public float getStaminaSpeedMultiplier() {
     return map(stats[1], 0.0, 1.0, 1.07, 0.93);
  }
  ...
  public float getSpeedBonus() { // -10% at 0, +10% at 1
    return map(stats[0], 0.0, 1.0, 0.9, 1.1);
  }
  ...
  public float getMetabolicSpeedMultiplier() {
    return map(stats[2], 0.0, 1.0, 0.9, 1.1); //-10% at 0, +10% at 1
  }
