class Player{
  ...
  void doPriorities(){
    ...
    if(ticks%TICK_BUCKET_COUNT == tick_bucket){
      ...
      if(predator == null){
        ...
        trait.priorities[4] = min(max((dist(0,0,dx,dy)-1)/3.5,PRIORITY_CAPS[4]),1);
      }
    }
  ...
  boolean isPreferredFood(int mySpecies, int targetSpecies) {
    ...
    if (mySpecies == 7) return (targetSpecies == 6 || targetSpecies == 2 || targetSpecies == 3); // Red Predator targets all Cows
    return false;
  }

  Player searchSameSpeciesMate() {
  ...
  Player searchAnyCowMate() {
    Player recordHolder = null;
    float distanceRecord = VISION_DISTANCE + trait.getVisionOffset();
    int[] cowSpecies = {2, 3, 6};

    for (int i = 0; i < players.size(); i++) {
      Player other = players.get(i);
      if (other == this) continue;
      boolean isCow = false;
      for (int s : cowSpecies) {
        if (other.species == s) { isCow = true; break; }
      }
      if (!isCow) continue;
  ...
  Player findTarget(int targetType) { 
    if (targetType == 0) { 
      Player sameSpeciesMate = searchSameSpeciesMate();
      if (sameSpeciesMate != null) return sameSpeciesMate;

      // Interbreeding fallback: search for any cow species if no same-species cow was found
      if (species == 2 || species == 3 || species == 6) {
        return searchAnyCowMate();
      }
    ...
    if (targetType == 2) {
      ...
      for (int i = 0; i < players.size(); i++) {
        ...
        if ((targetType == 2 && !IS_FOOD[other.species][species])) continue;
    ...
    if (species == 2) {        // Yellow Cow
      ...
      tertiaryFood = 6;  // Green Cow
    } else if (species == 7) { // Red/Green Predator
      // Search all cow species as primary target
      Player c6 = searchFoodBySpecies(6); // Green Cow
      Player c2 = searchFoodBySpecies(2); // Yellow Cow
      Player c3 = searchFoodBySpecies(3); // Teal Cow

      Player closestCow = null;
      float closestDist = Float.MAX_VALUE;

      Player[] cows = {c6, c2, c3};
      for (Player cow : cows) {
        if (cow != null) {
          float d = d_loop(coor, cow.coor, false);
          if (d < closestDist) {
            closestDist = d;
            closestCow = cow;
          }
      ...
      if (closestCow != null) return closestCow;
    }
  ...
  void doPhysics(Map map){
    ...
    if(topPriority == 2 && target != null && species >= 0 && target.species >= 0){ 
      boolean bothCows = (species == 2 || species == 3 || species == 6) && 
                        (target.species == 2 || target.species == 3 || target.species == 6);

      if (species == target.species || bothCows) {
        float _dist = dist(coor[0],coor[1],coor[2],target.coor[0],target.coor[1],target.coor[2]);
        if(_dist < COLLISION_DISTANCE){ 
          ...
          float offspringRoll = random(0,100);
          int numOffspring = 1;
          if(offspringRoll < 50){ // reworked number of offspring
            numOffspring = 1;
          }else if(offspringRoll < 87.5){
            numOffspring = 2;
          }else if(offspringRoll < 100){
            numOffspring = 3;
          }
          for (int k = 0; k < numOffspring; k++) {
            int offspringSpecies;

            if (species == target.species) {
              offspringSpecies = species;
              float mutationChance = random(0,25); 
              float r = random(0,1);
          
              if(mutationChance > 24){
               if(species == 2)
                offspringSpecies = 6;
               else if(species == 3)
                offspringSpecies = 6;
              ...
              if (roll < 46.0) {
                offspringSpecies = species;        // 46% Parent 1
              } else if (roll < 92.0) {
                offspringSpecies = target.species; // 46% Parent 2
              } else {
                // 8% Neither parent's species -> pick third cow species
                int[] allCows = {2, 3, 6};
                offspringSpecies = species;
                for (int c : allCows) {
                  if (c != species && c != target.species) {
                    offspringSpecies = c;
                    break;
                  }
                }
